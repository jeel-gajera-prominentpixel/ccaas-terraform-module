...
locals {
  module_project_path = "ccaas-terraform-modules-wrapper/jb-terraform-aws-cloudtrail"
  module_version      = "1.0.0"
}

locals {
  log_group = format("%s-log-grp-%s-%s-%s-%s", var.prefix_company, var.lob, var.application, var.prefix_region, var.env)
  tags = merge(
    var.tags,
    {
